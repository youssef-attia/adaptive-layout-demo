...
class _AdaptiveLayoutState extends State<AdaptiveLayout> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    bodyNotifier.value = widget.rightBody?.isActive;
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> entries = slots.entries
        ...
        .whereType<Widget>()
        .toList();
    bodyNotifier.value = widget.rightBody!=null?(widget.rightBody!.config.entries.first.key < MediaQuery.of(context).size.width):null;
    ...
    if (hasChild('body') && hasChild('rightBody')) {
      ...
      if (!slots['rightBody']!.isActive) {
        ...
        if (bodyRatio == null) {
          ...
          layoutChild(
              'rightBody', BoxConstraints.tight(Size(size.width / 2, size.height - bodyOffsetRB.dy - bodyOffsetLT.dy)));
        } else {
          currentSize = layoutChild(
              'body',
              BoxConstraints.tight(Size(
      ...
      positionChild('rightBody', Offset(bodyOffsetLT.dx + currentSize.width, bodyOffsetLT.dy));
